import * as THREE from '@/helpers/three-setup';
import { adapter, type UseCanvasResult } from "@minisheep/three-platform-adapter";
import { ray as _ray, settings } from "./core/settings.ts";
...
Page({
  ...
  onReady() {
    adapter.useCanvas(`#in-spirit-canvas`, this).then(this.useCanvas);
  },
  defaultHandler() {

  },
  getRainbowColor(percentage: number) {
    // 确保阈值在 0 到 100 之间
  ...
  useCanvas({ canvas, useFrame, eventHandler }: UseCanvasResult) {
    this.defaultHandler = eventHandler as any;
    ...
    this.data.enableFPS = !!supportedFloatType;
    ...
    function render(dt: number) {
      ...
      if (deltaTime > 1000) {
        ctx.setData({
          fps:(frames / (deltaTime / 1000)).toFixed(1)
        });
        frames = 0;
        prevUpdate = Date.now();
      }

      const targetColor = ctx.getRainbowColor(Math.floor((Date.now() / (10 * colorCycle)) % 100));
      settings.color2 = targetColor;

      _bgColor.setStyle(settings.bgColor);
      const tmpColor = ((floor as any).material as THREE.MeshStandardMaterial).color;
