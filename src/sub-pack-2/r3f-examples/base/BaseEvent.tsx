...
export function BaseEvent() {
  ...
  const onClick = useCallback((ev: ThreeEvent<MouseEvent>) => {
    (ev.object as THREE.Mesh<THREE.BoxGeometry, THREE.MeshToonMaterial>).material.color.set('#008080');
  }, [])
  const onPointerEnter = useCallback((ev: ThreeEvent<MouseEvent>) => {
    (ev.object as THREE.Mesh<THREE.BoxGeometry, THREE.MeshToonMaterial>).material.color.set('#DFFF45');
  }, [])
  const onPointerLeave = useCallback((ev: ThreeEvent<MouseEvent>) => {
    (ev.object as THREE.Mesh<THREE.BoxGeometry, THREE.MeshToonMaterial>).material.color.set('#efefef');
  }, [])
