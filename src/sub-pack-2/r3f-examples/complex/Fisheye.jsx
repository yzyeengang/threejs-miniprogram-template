...
import { Fisheye, PerspectiveCamera, Environment, MeshWobbleMaterial, useGLTF, OrbitControls } from '@react-three/drei'
import * as THREE from 'three'
import { useEffect, useRef, useState } from 'react'
import { useSpring, a } from '@react-spring/three'
import { CDN_PREFIX } from "@/constants";

export default function FisheyeApp({ eventSource }) {
  return (
    <Canvas flat eventSource={eventSource}>
      <Fisheye zoom={0}>
        ...
        <Environment files={`${CDN_PREFIX}/potsdamer_platz_1k.hdr`} background blur={1}/>
...
export function Level() {
  const { nodes } = useGLTF(`${CDN_PREFIX}/level-react-draco.glb`,"draco/gltf/")
  return <mesh geometry={nodes.Level.geometry} material={nodes.Level.material} position={[-0.38, 0.69, 0.62]}
...
export function Camera() {
  const { nodes, materials } = useGLTF(`${CDN_PREFIX}/level-react-draco.glb`,"draco/gltf/")
