...
import { withCDNPrefix } from "@/constants";


export default function MonitorsApp({eventSource}) {
  return (
    <Canvas shadows dpr={[1, 1.5]} camera={{ position: [-1.5, 1, 5.5], fov: 45, near: 1, far: 20 }} eventSource={eventSource}>
      {/* Lights */}
...
function Bun(props) {
  const { nodes } = useGLTF(withCDNPrefix('/bunny.glb'), 'draco/gltf/')
  return (
