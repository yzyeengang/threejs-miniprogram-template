...
import { useGLTF, useEnvironment, Text, Mask, useMask } from '@react-three/drei';
import { useRef } from 'react';
import { cdn, DecoderPath } from '@/constants';

const FONT_URL = `${cdn.R3F_ASSETS}inter_regular.woff`;
...
function Scene(props) {
  ...
  const { nodes, materials } = useGLTF(
    `${cdn.R3F_ASSETS}kitchen-transformed_png-texture.glb`,
    DecoderPath.GLTF
  );
  // Load environment (using it only on the chairs, for reflections)
  const env = useEnvironment({ files: `${cdn.R3F_ASSETS}potsdamer_platz_1k.hdr` });
  // Hover state
  ...
  // Get the priced item
  const price =
    { KNOXHULT: 5999, BRÖNDEN: 433, SKAFTET: 77, FANBYN: 255, VOXLÖV: 1699, LIVSVERK: 44 }[
      hovered
    ] ?? 5999;
